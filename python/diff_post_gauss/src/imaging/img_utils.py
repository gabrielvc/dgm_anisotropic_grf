...
from imaging.motionblur import Kernel
...
class Unfolder:
    ...
    def __call__(self, x):
        ...
        patch2D = reshape_patch(
            patch1D, crop_size=self.crop_size, dim_size=self.dim_size
        )
        return patch2D
...
class Folder:
    ...
    def __call__(self, patch2D):
        patch1D = reshape_patch_back(
            patch2D, crop_size=self.crop_size, dim_size=self.dim_size
        )
...
class exact_posterior:
    ...
    def pxt_given_x0(self, x0, xt, t, verbose=False):
        ...
        exp_in = (
            -1 / (2 * beta_t) * torch.linalg.norm(xt - np.sqrt(1 - beta_t) * x0) ** 2
        )
        if not verbose:
            return norm_const * torch.exp(exp_in)
        else:
    ...
    def prod_logsumexp(self, x0, xt, y, A, t):
        ...
        pxt_given_x0_density, pxtx0_nc, pxtx0_ei = self.pxt_given_x0(
            x0, xt, t, verbose=True
        )

...
# XXX added to support random mask
class RandomMask(Inpainting):
    def __init__(self, rate=0.95, n_channels=3, im_shape=256, device="cpu"):

        n_px = im_shape**2
        n_zero_px = int(rate * n_px)
        missing_px = torch.tensor(
            [
                i
        ...
        missing_indices = torch.concat(
            [n_channels * missing_px + k for k in range(n_channels)]
        )

        super().__init__(n_channels, im_shape, missing_indices, device)

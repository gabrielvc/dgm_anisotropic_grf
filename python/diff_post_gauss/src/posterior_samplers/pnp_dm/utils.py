...
def get_denoiser(epsilon_net: EpsilonNet, mode: str, use_ode: bool = True):
    """Convert EpsilonNet to a denoiser compatible with PNP-DM algorithm.

    mode : str
        Specifies how the denoiser is applied.
        Either "vp", "ve", "iddpm", "edm".
    """
    # XXX case-by-base for sound model
    # TODO: to improved be later to support native
    if isinstance(epsilon_net.net, SoundModelWrapper):
        model = VEPrecond(epsilon_net.net.denoiser, epsilon_net.net.sigmas)
    else:
        # map (DDPM)-epsilon_net to EDM convention
        model = VPPrecond(epsilon_net.net, epsilon_t=1e-3)

    n_steps = len(epsilon_net.timesteps) - 1
    device = epsilon_net.alphas_cumprod.device

    # load mode configs
    with open(
        REPO_PATH / "src" / "posterior_samplers" / "pnp_dm" / "mode_config.yml"
    ) as f:
        mode_configs = yaml.safe_load(f)

    denoiser = Denoiser_EDM(
        net=model,
        num_steps=n_steps,
        device=device,
        mode="pfode" if use_ode else "sde",
        **mode_configs[mode],
    )
...
# --- code copy/paste from https://github.com/zihuiwu/PnP-DM-public/blob/master/pnpdm/samplers/pnp_edm/denoiser_edm.py
# fmt: off
class Denoiser_EDM:
    def __init__(
        self,
        ...
        if scaling == "vp":
            ...
            self.s_deriv = (
                lambda t: -self.sigma(t) * self.sigma_deriv(t) * (self.s(t) ** 3)
            )
        else:
            assert scaling == "none"
            self.s = lambda t: 1
    ...
    def __call__(self, x_noisy, eta):
        i_start = torch.min(
            torch.nonzero(self.sigma(self.t_steps) < eta)
        ...
        for i, (t_cur, t_next) in enumerate(
            zip(self.t_steps[:-1], self.t_steps[1:])
        ):  # 0, ..., N-1
            if i < i_start:
            ...
            denoised = self.net(x_cur / self.s(t_cur), self.sigma(t_cur)).to(
                torch.float32
            )
            d_cur = (
                lmbd * self.sigma_deriv(t_cur) / self.sigma(t_cur)
                + self.s_deriv(t_cur) / self.s(t_cur)
            ) * x_cur - lmbd * self.sigma_deriv(t_cur) * self.s(t_cur) / self.sigma(
                t_cur
            ) * denoised
            ...
            if i != self.num_steps - 1 and self.mode == "sde":
                n_cur = (
                    self.s(t_cur)
                    * torch.sqrt(2 * self.sigma_deriv(t_cur) * self.sigma(t_cur))

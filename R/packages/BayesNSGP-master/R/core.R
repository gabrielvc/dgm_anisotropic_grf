...
nsCrosscorr <- nimble::nimbleFunction(
  run = function( Xdist1_sq = double(2), Xdist2_sq = double(2), Xdist12 = double(2), 
    ...
    if( Xdist2_sq[1,1] == -1 ){ # Isotropic case
      ...
      #   print(which(is.na(Dist.mat),arr.ind = T))
      #   print(Dist.mat)
      # }
    }
...
#' @export
#' 
nsgpPredict <- function(model, samples, coords.predict, predict.process = TRUE, constants, seed = 0, sgvOrdering=NULL, ... ) {
  ...
  if( modelsList$likelihood == "SGV" ) {
    if(is.null(sgvOrdering)){
      ...
      message("\nUsing supplied ordering for SGV.\n")
      predSGV_setup <- sgvOrdering
      pred.mmd.seed <- sgvOrdering$seed
    }
  ...
  if( modelsList$likelihood == "fullGP" ){ # Predictions for the full GP likelihood
    ...
    if( modelsList$Sigma == "covReg" | modelsList$Sigma == "compReg" | modelsList$Sigma == "custom" ){
      constants$PX_Sigma <- constants$PX_Sigma[pred_ord,]
    }
  ...
  nimCat("|-------------|-------------|-------------|-------------|")
  nimCat("\n|")
  kr_obs_orig=matrix(0,nrow = J,ncol=N)
  kr_pred_orig=matrix(0,nrow = J,ncol=M)
  ...
  for(j in 1:J){ # Loop over MCMC samples
    ...
    if( modelsList$tau == "fixed" ){
      # Required constants: tau_HP1
      log_tau_vec_j <- log(constants$tau_HP1)*rep(1,N)
      Plog_tau_vec_j <- log(constants$tau_HP1)*rep(1,M)
    }
    if( modelsList$tau == "logLinReg" ){
    ...
    if( modelsList$sigma == "fixed" ){
      # Required constants: none
      log_sigma_vec_j <- log(constants$sigma_HP1)*rep(1,N)
      Plog_sigma_vec_j <- log(constants$sigma_HP1)*rep(1,M)
    }
    if( modelsList$sigma == "logLinReg" ){
    ...
    if( modelsList$Sigma == "ganiso" ){
      # Required constants: None
      Sigma_coef1 <- samp_j["Sigma_coef1"]
      Sigma_coef2 <- samp_j["Sigma_coef2"]
      Sigma_coef3 <- samp_j["Sigma_coef3"]
      
      eigen_comp1_j <- ((Sigma_coef1/4)**2)*rep(1,N)
      eigen_comp2_j <- ((Sigma_coef1*Sigma_coef2/4)**2)*rep(1,N)
      eigen_comp3_j <- (Sigma_coef3)*rep(1,N)
      
      Sigma11_j <- as.numeric(inverseEigen_ganiso(eigen_comp1_j, eigen_comp2_j, eigen_comp3_j, 1))
      Sigma12_j <- as.numeric(inverseEigen_ganiso(eigen_comp1_j, eigen_comp2_j, eigen_comp3_j, 3))
      Sigma22_j <- as.numeric(inverseEigen_ganiso(eigen_comp1_j, eigen_comp2_j, eigen_comp3_j, 2))
      
      Peigen_comp1_j <- ((Sigma_coef1/4)**2)*rep(1,M)
      Peigen_comp2_j <- ((Sigma_coef1*Sigma_coef2/4)**2)*rep(1,M)
      Peigen_comp3_j <- (Sigma_coef3)*rep(1,M)
      
      PSigma11_j <- as.numeric(inverseEigen_ganiso(Peigen_comp1_j, Peigen_comp2_j, Peigen_comp3_j, 1))
      PSigma12_j <- as.numeric(inverseEigen_ganiso(Peigen_comp1_j, Peigen_comp2_j, Peigen_comp3_j, 3))
      PSigma22_j <- as.numeric(inverseEigen_ganiso(Peigen_comp1_j, Peigen_comp2_j, Peigen_comp3_j, 2))
      
    }
    if( modelsList$Sigma == "custom" ){
      # Required constants: X_Sigma, PX_Sigma
      X_Sigma <- constants$X_Sigma
      PX_Sigma <- constants$PX_Sigma
      ...
      Sigma_coef1 <- samp_j["Sigma_coef1"]
      Sigma_coef2 <- samp_j["Sigma_coef2"]
      Sigma_coef3 <- samp_j[paste0("Sigma_coef3[",1:ncol(X_Sigma),"]")]
      
      eigen_comp1_j <- ((Sigma_coef1/4)**2)*rep(1,N)
      eigen_comp2_j <- ((Sigma_coef1*Sigma_coef2/4)**2)*rep(1,N)
      eigen_comp3_j <- (X_Sigma%*%Sigma_coef3) 
      
      Sigma11_j <- as.numeric(inverseEigen_custom(eigen_comp1_j, eigen_comp2_j, eigen_comp3_j, 1))
      Sigma12_j <- as.numeric(inverseEigen_custom(eigen_comp1_j, eigen_comp2_j, eigen_comp3_j, 3))
      Sigma22_j <- as.numeric(inverseEigen_custom(eigen_comp1_j, eigen_comp2_j, eigen_comp3_j, 2))
      
      
      Peigen_comp1_j <- ((Sigma_coef1/4)**2)*rep(1,M)
      Peigen_comp2_j <- ((Sigma_coef1*Sigma_coef2/4)**2)*rep(1,M)
      Peigen_comp3_j <- (PX_Sigma%*%Sigma_coef3) 
      
      PSigma11_j <- as.numeric(inverseEigen_custom(Peigen_comp1_j, Peigen_comp2_j, Peigen_comp3_j, 1))
      PSigma12_j <- as.numeric(inverseEigen_custom(Peigen_comp1_j, Peigen_comp2_j, Peigen_comp3_j, 3))
      PSigma22_j <- as.numeric(inverseEigen_custom(Peigen_comp1_j, Peigen_comp2_j, Peigen_comp3_j, 2))
    ...
    if( modelsList$likelihood == "fullGP" ){ # Predictions for the full GP likelihood
      ...
      PPD_obs_ord <- matrix(postPredDraws[,1:N],ncol=N)
      PPD_pred_ord <- matrix(postPredDraws[,-(1:N)],ncol=ncol(postPredDraws)-N)
      PPD_obs_orig <- PPD_obs_ord[,order(model_constants$ord)]
      PPD_pred_orig <- PPD_pred_ord[,order(predSGV_setup$ord_pred)]
      
      kr_obs_orig[j,] <- krigPredictor[1:N][order(model_constants$ord)]
      kr_pred_orig[j,] <- krigPredictor[-(1:N)][order(predSGV_setup$ord_pred)]
  ...
  if( modelsList$likelihood == "fullGP" ){ # Predictions for the full GP likelihood
    ...
    output <- list(obs = PPD_obs_orig, pred = PPD_pred_orig, pred.mmd.seed = pred.mmd.seed,
                   krig_obs=kr_obs_orig,krig_pred=kr_pred_orig) 
  } else stop('')

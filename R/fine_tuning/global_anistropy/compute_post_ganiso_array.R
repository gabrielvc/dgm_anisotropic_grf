...
id_run=as.numeric(args[5])
nArrayRuns=as.numeric(args[6])+1

n_samples_max=10000
if(is.null(n_samples_max)){
n_samples_max=Inf
}
...
mc.cores=20
nBatch=500 ## Number of samples computed in parallel
plt=F
...
param_list=param_list[(ipp[id_run+1]+1):(ipp[id_run+2]),]

...
id_file=as.numeric(args[1]) ## For seed
if(sum(is.na(c(id_file)))>0){
  stop("Two numerical argumuments are expected")
}
suffix_name=args[2] # Name of the folder containing the exports
...
for(k in 1:nex){
  ...
  a_mean=0.1;
  log_a_std=0.1
  log_a_mean = log(a_mean)-log_a_std**2/2
  a=exp(log_a_mean+log_a_std*rnorm(1))
